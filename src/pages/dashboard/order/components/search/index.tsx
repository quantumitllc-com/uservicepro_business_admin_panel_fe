        ...
        content={({ close }) => (
          ...
          >
            <Pane flexGrow={1} overflowY='auto'>
              {search === ""
                ? data?.data
                    .filter(({ id }: any) => !list.find((v) => v.id === id))
                    .map(({ id, name }: any) => (
                      <Checkbox
                        key={id}
                ...
                : data?.data
                    .filter(
                      ({ id, name }: any) =>
                        name.includes(search) || !list.find((v) => v.id === id)
                    )
                    .map(({ id, name }: any) => (
                      <Checkbox
                        key={id}
            ...
            <Pane marginY='10px' display='flex' justifyContent='flex-end'>
              <Button
                appearance='primary'
                onClick={() => {
                  mutate(undefined, {
      ...
      >
        <SearchInput
          ...
          placeholder={
            data?.data.length === 0 ||
            data?.data.length === 0 ||
            data?.data?.filter(({ id }: any) => !list.find((v) => v.id === id))
              .length === 0
