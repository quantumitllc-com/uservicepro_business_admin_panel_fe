					...
					{tabs.map((tab, index) => (
						...
						>
							<Pane
								...
								gap="12px"
							>
								{index === selectedIndex ? tab.selected : tab.icon} {tab.text}
							</Pane>
							{index === selectedIndex ? tab.arrowSelected : <Pane marginRight={minorScale(2)}>{tab.arrow}</Pane>}
						</Tab>
					))}
				</Tablist>
			</nav>
				...
				<Heading size={700} fontWeight={500} fontSize={36} marginBottom={majorScale(6)}>
					...
					<Pane className={styles.accordionContainer}>
						<ul className={styles.accordion}>
							{accordionDataBusiness.map(({ heading, icon, iconSelected, badge, success, text, start, verify  }, index) => (
								<Accordion
									key={index}
									icon={icon}
