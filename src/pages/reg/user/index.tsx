	...
	return (
		<Pane className={styles.container}>
				...
				>
					{tabs.map((tab, index) => (
						<Tab
							...
							backgroundColor={index === selectedIndex ? "transparent!important" : ""}
							color={index === selectedIndex ? "var(--black)!important" : "var(--grey)"}
						...
						>
							<Pane
								...
								gap="12px"
							>
								{(index === selectedIndex  || index === 3) ? tab.selected : tab.icon} {tab.text}
							</Pane>
							{(index === selectedIndex  || index === 3) ? tab.arrowSelected : <Pane marginRight={minorScale(2)}>{tab.arrow}</Pane>}
						</Tab>
					))}
				</Tablist>
			</nav>
			...
			<main className={styles.main}>
				<Heading size={700} fontWeight={500} fontSize={36} marginBottom={majorScale(6)}>
					Get Started, Clark
				</Heading>
					<Pane className={styles.accordionContainer}>
						<ul className={styles.accordion}>
							{accordionData.slice(0,-1).map(({ heading, icon, iconSelected, badge, success, text, start, verify  }, index) => (
								<Accordion
									key={index}
									icon={icon}
